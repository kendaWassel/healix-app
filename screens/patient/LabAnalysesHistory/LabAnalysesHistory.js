...
export default function LabAnalysesHistory() {
  ...
  const openOriginalFile = async (url) => {
    if (!url) {
      setError("Original test file is not available.");
      return;
    }
    ...
    try {
      ...
      setError("Unable to open the original test file.");
    }
  };
    ...
    return (
      <View style={styles.card}>
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            downloadPdf(
              item.patient_pdf_url,
              item.id,
              t("labHistory.downloadDialogTitle"),
            )
          }
          ...
          {isDownloading ? (
            ...
            <>
              ...
              <Text style={styles.downloadBtnText}>
                {t("labHistory.download")}
              </Text>
            </>
          )}
        </TouchableOpacity>
        <TouchableOpacity
          style={styles.originalFileBtn}
          onPress={() => openOriginalFile(item.original_file_url)}
        ...
        >
          ...
          <Text style={styles.originalFileBtnText}>
            {t("labHistory.viewTest")}
            </Text>
        </TouchableOpacity>
      </View>
    );
  ...
  <View style={styles.disclaimerHeader}>
    ...
    >
      <Text style={styles.disclaimerToggleText}>
        {showDisclaimer
          ? t("labHistory.seeLess")
          : t("labHistory.seeMore")}
      </Text>
...
disclaimerText: {
  ...
  subtitle: { color: colors.gray500, marginTop: 6, marginBottom: 16 },
